...
"skewt.axis" <- function(BROWN = "brown", GREEN = "green", redo = FALSE, ...) {
    ...
    plot(xc, yc,
      type = "l", axes = FALSE, xlab = "", ylab = "", lwd =
        0.10000000000000001
    )
    ...
    axis(1,
      at = skewtx(degc, ypos), labels = seq(-20, 100, by = 20), pos
      = ymax
    )
    ...
    y <- skewty(pres)
    segments(xpl, y, xpr, y,
      col = BROWN, lwd = 0.10000000000000001, lty =
        2
    )
    ...
    xl <- skewtx(tmix, yl)
    text(xl, yl,
      labels = as.character(mixrat[1]), col = GREEN, srt = 55,
      adj = 0.5, cex = 0.75
    )
    ...
    xl <- skewtx(tmix, yl)
    text(xl, yl,
      labels = as.character(mixrat[2:NMIX]), col = GREEN, srt =
        55, adj = 0.5, cex = 0.75
    )
    #---------------------------------------------------------------------
    # --- DRAW DRY ADIABATS.
    ...
    for (ipseudo in 1:NPSEUDO) {
      ...
      if (labelx > xmin) {
        text(labelx, labely,
          labels = as.character(pseudo[ipseudo]),
    ...
    invisible(list(
      pseudox = holdx, pseudoy = holdy, pseudo = pseudo,
